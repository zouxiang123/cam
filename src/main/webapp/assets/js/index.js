...
function addEvents() {
    ...
    $("[data-permissionurl]").click(function(event) {
        ...
        var param = $(this).data("permissionurl");
        if (!isEmpty(param)) {
    ...
    $("[data-openurl]").click(function(event) {
        var url = convertEmpty($(this).data("baseurl")) + "/" + $(this).data("openurl");
        var openUrl = ctx + "/openUrl.shtml?redirectUrl=" + encodeURIComponent(url);
        $("#frameBody").attr("src", openUrl);
        stopEventBubble(event);
    });

    $("html").on("click", function(e) {
        if (!$(e.target).hasClass("u-user")) {
            $("#settingId").hide();
        }
    });
...
function resizeFrame() {
    if (isExtend) {
        ...
        $("#frameBody").css("width", $(window).width() - 172);
    } else {
        $("#frameBody").css("left", 46);
        $("#frameBody").css("width", $(window).width() - 46);
    }
    $("#frameBody").css("height", $(window).height());
}
